...
const float EGAAudioRecorderSampleRate = 16000;
...
static OSStatus recordingCallback(void *inRefCon,
    ...
    if (status == noErr) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [recorder->_delegate audioRecorder: recorder data: _mData didRecordAudioAtPeakAudioLevel: recorder->_peakAudioLevel withUpdatedDuration: recorder->_duration];
        });
...
- (void)prepareAudioSession {
    ...
    [audioSession setCategory: AVAudioSessionCategoryPlayAndRecord
                  withOptions: AVAudioSessionCategoryOptionAllowBluetooth | AVAudioSessionCategoryOptionAllowBluetoothA2DP | AVAudioSessionCategoryOptionDefaultToSpeaker
                        error: &error];
    [audioSession setPreferredSampleRate: EGAAudioRecorderSampleRate error: &error];
